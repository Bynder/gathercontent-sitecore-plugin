define([
    ...
    var ctor = function () {
        ...
        this.updateNodeToClientAction = function ($el, itemData, select) {
            ...
            // add item uri data
            $el.attr('data-sc-id', itemData.ItemID);
            $el.attr('data-sc-version', itemData.ItemVersion);
            $el.attr('data-sc-revision', itemData.__Revision);
            this.wrapNode($el, this.clientActionKey, itemData.ItemName);
            _scl.PageModes.ChromeManager.resetChromes();
            if (select) {
                this.selectChromeByDisplayNameAndType(itemData.ItemName, this.clientActionKey);
            }
        }

        this.updateNodeToPlaceholder = function ($el, itemData, position) {

            var itemGuid = '{' + itemData.ItemID + '}';

            $el.attr('data-sc-id', itemGuid);
            $el.attr('data-sc-version', itemData.ItemVersion);
            $el.attr('data-sc-revision', itemData.__Revision);

            var contentId = "content_" + itemData.ItemID;
            ...
            switch (position) {
                ...
                default:
                    ...
                    $el.attr("placeholderKey", itemGuid);
            ...
            var pl = $sc("#" + contentId);
            this.wrapNode(pl, this.placeholderKey, itemGuid, itemData.ItemName);
            _scl.PageModes.ChromeManager.resetChromes();
            this.selectChromeByDisplayNameAndType(itemData.ItemName, this.placeholderKey);
        }
