var UpdateManager = function () {
    ...
    self.search = function (currentCollection) {
        ...
        if (value !== '') {
            ...
            for (var i = 0; i < currentCollection.length; i++) {
                ...
                if (currentElement.CwbItem.Name.toLowerCase().indexOf(value.toLowerCase()) >= 0) {
    ...
    self.filterByProject = function (currentCollection) {
        ...
        if (value) {
            ...
            for (var i = 0; i < currentCollection.length; i++) {
                var currentElement = currentCollection[i];
                if (currentElement.CwbProject.Name === value) {
                    resultCollection.push(currentElement);
                }
            }
        }
    ...
    self.filterByTemplate = function (currentCollection) {
        ...
        if (value) {
            ...
            for (var i = 0; i < currentCollection.length; i++) {
                var currentElement = currentCollection[i];
                if (currentElement.CwbTemplate.Id === value) {
                    resultCollection.push(currentElement);
                }
            }
        }
    ...
    var options =
        {
            ...
            columnDefs: [
                ...
                { field: 'CwbItem.Name', width: '**', displayName: 'Content Workflow Item Name' },
                { field: 'CwbProject.Name', width: '**', displayName: 'Content Workflow Project' },
                {
                    field: 'LastUpdatedInCwb', width: '**', displayName: 'Last updated in Content Workflow',
                    sortFn: dateSort
                },
                {
                ...
                { field: 'CwbTemplate.Name', width: '**', displayName: 'Content Workflow Template' },
                ...
                { displayName: 'Open in Content Workflow', width: 70, cellClass: 'cell-padding', sortable: false, cellTemplate: '<a data-bind="click: function(){$parent.$userViewModel.openCwbLink($parent.entity)}">Open</a>' }
            ],
        };
    ...
    var confirmOptions =
          {
              ...
              columnDefs: [
                ...
                { displayName: 'Open in Sitecore', cellClass: 'cell-padding', sortable: false, cellTemplate: '<a data-bind="if: $parent.entity.CmsLink!=null, click: function(){$parent.$userViewModel.openCmsLink($parent.entity)}">Open</a>' },
                  { displayName: 'Open in Content Workflow', cellClass: 'cell-padding', sortable: false, cellTemplate: '<a data-bind="click: function(){$parent.$userViewModel.openCwbLink($parent.entity)}">Open</a>' }
              ]
          };
