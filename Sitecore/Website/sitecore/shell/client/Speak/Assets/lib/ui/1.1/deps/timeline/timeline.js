 ...
 * TODO
 *
 * Add zoomzooming with pinching on Android
 *
...
links.Timeline = function (container, options) {
  ...
  this.options = {
    'width': "100%",
    'height': "auto",
    'minHeight': 500, // minimal height in pixels
    'groupMinHeight': 0,
    'autoHeight': true,

    'eventMargin': 10, // minimal margin between events
    'eventMarginAxis': 50, // minimal margin between events and the axis
    'eventMarginX': 0, // minimal horizontal margin between events
    'dragAreaWidth': 50, // pixels
    ...
    'axisOnTop': false,
    'stackEvents': true,
    'maxStackHeight': 100, //The maximum number of levels of displacement for overlapping items
    'stackOverflow': true, //Allow colliding items to be placed outside the visible area. Specify a number for the maximum number of pixels allowed.
    'animate': true,
    'animateZoom': true,
    'cluster': false,
    clusterFormatter: function (cluster) {
      var title = 'Cluster containing ' + cluster.items.length +
        ' events. Zoom in to see the individual events.';
      var content = '<div title="' + title + '">' + cluster.items.length + ' events</div>';
  ...
  (function f() {
    window.requestAnimationFrame(function () {
      if (_this.updateAnimation) _this.updateAnimation();
      f();
    });
  })();
...
links.Timeline.prototype.getItemIndex = function (item) {
 ...
 *                          {String} group
 */
links.Timeline.prototype.updateData = function (item, values) {
  var data = this.data,
      prop;
...
links.Timeline.prototype.getItemFromElement = function (element) {
  var e = element,
      dom = this.dom,
      frame = dom.items.frame; //try to find the item from element data
  while (e.parentNode && e !== frame) {
    var data = $(e).data("timeline-event");
    if (data) return data;
    e = e.parentNode;
  }
...
links.Timeline.prototype.render = function (options) {
  ...
  var zoom = (1 * this.end - 1 * this.start) / this.size.contentWidth;
...
links.Timeline.prototype.processItems = function (delay) {
  clearTimeout(this._processTimeout);
  if (delay) {
    var _this = this;
    this._processTimeout = setTimeout(function () { _this.processItems(false); }, 1);
  } else {
    var processor = this.options.itemProcessor;
    if (processor) {
      processor(this.items, this.clusters, this);
    }
  }
}; /**
...
links.Timeline.prototype.repaintFrame = function () {
  ...
  $(dom.content).css("css3left", size.contentLeft + "px");
...
links.Timeline.prototype.reflowAxis = function () {
  ...
  var characterMinorWidth = (axisDom && axisDom.characterMinor) ? axisDom.characterMinor.clientWidth : 0,
      ...
      (options.showMajorLabels ? characterMajorHeight : 0) +15; 

  var axisTop = options.axisOnTop ? 0 : size.frameHeight - axisHeight;
  var axisLine = options.axisOnTop ? axisHeight : axisTop;
...
links.Timeline.prototype.repaintEras = function () {
  dom = this.dom;
  var frame = dom.eras;
  if (!frame) {
    frame = document.createElement("DIV");
    frame.className = "timeline-eras";
  ...
  for (var i = 0; i < this.options.eras.length; i++) {
    var era = this.options.eras[i];

    if (!era.el) {
      era.el = document.createElement("DIV");
      era.el.style.position = "absolute";
      era.el.style.top = "0px";
      era.el.className = "timeline-era " + (era.cssClass || "");
      dom.eras.appendChild(era.el);

      era.label = document.createElement("DIV");
      era.label.className = "timeline-era-label";
      era.label.style.position = "absolute";
      era.el.appendChild(era.label);
      era.label.innerHTML = era.content;
    }

    var start = era.start || this.start;
    var end = era.end || this.end;
    if (start > end) start = end;

    if (start < this.start && end > this.start) {
      era.label.style.left = (this.timeToScreen(this.start) - this.timeToScreen(start)) + "px";
    } else {
      era.label.style.left = 0;
    }

    if (era.label.offsetWidth > this.timeToScreen(end) - this.timeToScreen(this.start)) {
      era.label.style.left = (this.timeToScreen(end) - era.label.offsetWidth) + "px";
    }

    era.el.style.left = this.timeToScreen(start) + "px";
    era.el.style.width = (this.timeToScreen(end) - this.timeToScreen(start)) + "px";

//console.log(era.el.style.left + ": " + era.el.style.width);
  }
}; /**
...
links.Timeline.prototype.repaintAxisMinorText = function (x, text) {
  ...
  $(label).css("css3left", x);
  label.style.top = size.axis.labelMinorTop + 4 + "px";
...
links.Timeline.prototype.repaintAxisMinorLine = function (x) {
  ...
  line.style.top = size.contentHeight + "px";
  line.style.height = (axis.lineMinorHeight - size.contentHeight) - 17 + "px";
  //line.style.left = (x - axis.lineMinorWidth/2) + "px";
  $(line).css("css3left", (x - axis.lineMinorWidth / 2));
...
links.Timeline.prototype.repaintAxisMajorText = function (x, text) {
  ...
  label.childNodes[0].nodeValue = text;
  label.style.top = size.axis.labelMajorTop - 3 + "px";
  $(label).css("css3left", x);
  //label.style.left = x + "px";
  //label.title = title; // TODO: this is a heavy operation
...
links.Timeline.prototype.repaintAxisMajorLine = function (x) {
  ...
  line.style.top = size.contentHeight + "px";
  //line.style.height = (axis.lineMinorHeight - size.contentHeight) + "px";
  line.style.height = (size.frameHeight - size.contentHeight) + "px";
  $(line).css("css3left", (x - axis.lineMajorWidth / 2));
...
links.Timeline.prototype.recalcItems = function () {
  ...
  if (groups.length == 0) {
    ...
    for (i = 0, iMax = groups.length; i < iMax; i++) {
      ...
      group.height = 20;// Math.max(groupHeight, options.groupMinHeight);
...
links.Timeline.prototype.repaintMarkerLine = function () {
  if (!this.options.showMarkerLine) return;

  var dom = this.dom;
  if (!dom.markerLine) {
    // create the current time bar
    var markerLine = document.createElement("DIV");
    markerLine.className = "timeline-markerline";
    markerLine.style.position = "absolute";
    markerLine.style.top = "0px";

    dom.contentTimelines.appendChild(markerLine);
    dom.markerLine = markerLine;
  }
  if (this.size && this.size.contentHeight) {
    dom.markerLine.style.height = this.size.contentHeight + "px";
  }

  dom.markerLine.style.left = this.timeToScreen(1 * this.start + (1 * this.end - 1 * this.start) / 2) + "px";
}; /**
  ...
  if (!navBar) {
    ...
    if (showButtonNew) {
      ...
      var onAdd = function (event) {
        ...
        var item = timeline.items[timeline.items.length - 1];
...
links.Timeline.prototype.onTouchStart = function (event) {
  ...
  if (!params.onTouchMove) {
    ...
    if ($(document).hammer()) {
      params.onTouchMove = function () {};

      $(document).hammer().on("transform drag", function (ev) {
        me.onTouchMove(ev.gesture);
      });
    } else {
  ...
  var target = links.Timeline.getTarget(event);
  var item = this.getItemFromElement(target);
...
links.Timeline.prototype.onTouchMove = function (event) {
  ...
  if (!params.zoomed) {
    // move		
    this.onMouseMove(event.srcEvent || event);
  } else {
    if (this.options.zoomable) {
      ...
      _this.updateAnimation = function () {
        _this.setVisibleChartRange(start, end);
        _this.trigger("rangechange");
        _this.updateAnimation = null;
      };
    }
  }
...
links.Timeline.prototype.onTouchEnd = function (event) {
  ...
  if (params.onTouchMove) {
    if ($(document).hammer()) {
      $(document).hammer().off("transform drag");
    } else {
  ...
  var target = links.Timeline.getTarget(event);
  var doubleTapItem = this.getItemFromElement(target);
...
links.Timeline.prototype.stopAnimation = function () {
  if (this.animationStopper) {
    var f = this.animationStopper;
    delete this.animationStopper;
    f();
  }
}; /**
...
// get mouse position
  ...
  if (params.itemDragLeft || params.itemDragRight) {
    params.item = this.selection;
  } else {
    params.item = this.getItemFromElement(params.target);
  }
  ...
  if (params.addItem) {
    ...
    params.item = this.items[this.items.length - 1];
    this.selectItem(params.item);
    params.itemDragRight = true;
  }

  var item = params.item;
  var isSelected = this.isSelected(params.item);
...
links.Timeline.prototype.setStart = function (newStart, animate, args, callback) {
  var _this = this;

  var start = 1 * this.start, end = 1 * this.end;
  var window = 1 * end - 1 * start;
  ...
  var args = args || {
    ...
    animating: animate,
    complete: function () {
      $(_this.dom.items.frame).css("css3left", "0px");
      _this.setVisibleChartRange(_this.start, _this.end);
      _this.stopAnimation();
      _this.updateAnimation = null;
    },
    bounds: { start: start - window, end: end + window }
  };
  ...
  if (animate) {
    var range = _this.getVisibleChartRange();
    var width = (1 * range.end - 1 * range.start);

    var center = 1 * newStart - width / 2;
    if (this.options.min && center < 1 * this.options.min) {
      center = 1 * this.options.min;
    } else if (this.options.max && center + width > this.options.max) {
      center = 1 * this.options.max - width;
    }

    var animation = $({ center: 1 * range.start }).animate({ center: center }, {
      easing: animate.easing ? animate.easing : this.options.animationEasing,
      duration: animate.duration ? animate.duration : this.options.animationSpeed,
      step: function (now, fx) {
        args = _this.setStart(new Date(now), false, args, true);
      },
      complete: function () { if (args) args.complete(); }
    });

    _this.animationStopper = function () {
      args.complete();
      animation.stop();
    };
  } else {

    var diffMillisecs = 1 * newStart - 1 * args.start;
    var newEnd = new Date(args.end + diffMillisecs);

    if (1 * newStart < args.bounds.start || 1 * newEnd > args.bounds.end) {
      //Now we are in an area where the dom elements for the item might not have been created.
      //Do the full render, and reset movement reference points
      _this.render();
      args = _this.setStart(newStart, false, null, true);
    } else {
      this.updateAnimation = function () {

        _this.applyRange(newStart, newEnd, null, true);
        _this.recalcConversion();

        var frameLeft = -diffMillisecs / window * _this.size.contentWidth;
        $(_this.dom.items.frame).css("css3left", frameLeft + "px");
        //this.dom.items.frame.style.left = Math.round(frameLeft) + "px";
        _this.repaintCurrentTime();
        _this.repaintCustomTime();
        _this.repaintMarkerLine();
        _this.repaintAxis();

        _this.applyRange(newStart, newEnd);
        _this.recalcConversion();
        _this.trigger('rangechange');
      };
    }
...
links.Timeline.prototype.onMouseMove = function (event) {
  ...
  if (params.lastMove) {
    //Pixels per second
    var timeDiff = (1 * (new Date()) - 1 * params.lastMove);
    if (timeDiff < 25) timeDiff = 25;

    var prevVelocity = params.velocity || 0;
    var velocity = (mouseX - (params.prevX || 0)) / (timeDiff / 1000);
    params.velocity = timeDiff < 250 ? (prevVelocity + velocity) / 2 : velocity;
  } else {
    params.velocity = 0;
  }
  params.lastMove = new Date();
  params.prevX = mouseX;
  ...
  if (params.customTime) {
    ...
    params.moveArgs = this.setStart(newStart, false, params.moveArgs);
  }
  ...
  if (params.customTime) {
    ...
    if (!params.moved && !params.zoomed) {
      ...
      if (params.target === this.dom.items.deleteButton) {
        ...
        if (params.item) {
          if (!this.isSelected(params.item)) {
            this.selectItem(params.item);
            this.trigger('select');
          }
        } else {
      ...
      if (params.moved && params.lastMove) {
        this.render({ animate: false });
        var diff = 1 * new Date() - 1 * params.lastMove;
        if (diff < 250) {
          var _this = this;
          var fps = 60;
          //Px/s to s/frame
          var velocity = -(params.velocity / _this.conversion.factor) / fps;
          ...
          (function f() {
            ...
            animationArgs = _this.setStart(new Date(1 * _this.start + velocity), false, animationArgs, true);
            velocity -= 0.05 * velocity;
            if (Math.abs(velocity * _this.conversion.factor) < 0.25) {
              _this.stopAnimation();
            } else {
              to = setTimeout(function () { f(); }, 1000 / fps);
            }
          })();

          _this.animationStopper = function () {
            clearTimeout(to);
            if (animationArgs) animationArgs.complete();
          }; /*var range = 1*params.end - 1*params.start;
					var move = range * -params.velocity/100;															
					this.setStart(new Date(1*this.start + range/2 + move), {duration: 1000, easing: "easeOutExpo"});*/
        }
      } else {
        // timeline is moved
...
links.Timeline.prototype.zoom = function (zoomFactor, zoomAroundDate) {
  ...
  if (interval >= zoomMin) {
    if (this.selection && this.selection.cluster && this.selection.cluster.dom) {
      this.selection.cluster.unselect();
    }
 ...
 */
links.Timeline.prototype.applyRange = function (start, end, zoomAroundDate, skipRangeCheck) {
  ...
  var min = !skipRangeCheck && options.min ? options.min.valueOf() : undefined; // number
  var max = !skipRangeCheck && options.max ? options.max.valueOf() : undefined; // number
 ...
 */
links.Timeline.prototype.confirmDeleteItem = function (item) {
 ...
 */
links.Timeline.prototype.deleteItem = function (item, preventRender) {
  var index = this.getItemIndex(item);
...
links.Timeline.Item = function (data, options) {
  if (data) {
    ...
    this.data = data; //NKJ: Added. We need that for rendering
  }
  this.top = 0;
  this.left = 0;
  this.width = 0;
...
links.Timeline.ItemBox.prototype.createDOM = function () {
  ...
  $(this.dom).add(this.dom.dot).add(this.dom.line).data("timeline-event", this); //NKJ
...
links.Timeline.ItemBox.prototype.updateDOM = function () {
  ...
  if (divBox) {
    ...
    if (this.data.cssClass) {
      links.Timeline.addClassName(divBox, this.data.cssClass);
      links.Timeline.addClassName(divLine, this.data.cssClass);
      links.Timeline.addClassName(divDot, this.data.cssClass);
    }
...
links.Timeline.ItemRange.prototype.createDOM = function () {
  ...
  this.dom = divBox;
  $(this.dom).data("timeline-event", this); //NKJ
  this.updateDOM();
...
links.Timeline.prototype.addItems = function (itemsData, preventRender) {
  ...
  itemsData.forEach(function (itemData) {
    ...
    var item = timeline.createItem(itemData);
    items.push(item);
    timeline.updateData(item, itemData);
...
links.Timeline.prototype.createItem = function (itemData) {
  ...
  var data = links.Timeline.clone(itemData); //TODO: This might as well just be the data used by the timeline (start, end, content, etc.)
...
links.Timeline.prototype.setSelection = function (dataItem, animate, id, displacement) {

  if (dataItem || id || displacement) {
    if (displacement && this.selection) {
      id = this.selection.data.interactionData.TimelineEventId;
    }

    var item = null;
    for (var i = 0; i < this.items.length; i++) {
      if (this.items[i].data.src == dataItem || this.items[i].data.interactionData.TimelineEventId == id) {
        var index = i + (displacement || 0);
    ...
    if (item) {
      ...
      var diff = (this.end.valueOf() - this.start.valueOf());
      if (this.options.min && middle - diff / 2 < 1 * this.options.min) {
        middle = this.options.min + diff / 2;
      } else if (this.options.max && middle + diff / 2 > 1 * this.options.max) {
        middle = this.options.max - diff / 2;
      }

      if (animate) {
        this.setStart(new Date(middle), true);
      } else {
        var
...
links.Timeline.prototype.selectItem = function (item) {
  this.unselectItem();

  if (item.isCluster) return; // Don't select clusters
  ...
  if (item) {
    ...
    if (item.cluster && item.cluster.dom) {
      item.cluster.select();
    }
    this.repaintDeleteButton();
    this.repaintDragAreas();
 ...
 */
links.Timeline.prototype.isSelected = function (item) {
  return (this.selection && this.selection == item);
};
...
links.Timeline.prototype.finalItemsPosition = function (items, groupBase, group) {
  ...
  for (i = 0, iMax = groupFinalItems.length; i < iMax; i++) {
    ...
    var shifts = 0; //The number of shifts upwards
    var overlaps = [];

    if (this.options.stackEvents && !finalItem.item.data.noStack) {
      var z = 100;
      do {
        ...
        //Look forward
        if (collidingItem == null && i < iMax - 1) {
          collidingItem = this.stackItemsCheckOverlap(groupFinalItems, i, i + 1, i + 1);
        }
        if (collidingItem != null) {
          /*collidingItem.item.dom.style.zIndex = z;
					z -= 2;*/

          overlaps.push({ item: collidingItem, top: finalItem.top });

          // There is a collision. Reposition the event above the colliding element
          if (axisOnTop) {
            finalItem.top = collidingItem.top + collidingItem.height + eventMargin;
          } else {
            var newTop = collidingItem.top - finalItem.height - eventMargin;
            if (this.options.stackOverflow !== true && newTop < -(this.options.stackOverflow || 0)) {
              break; //console.log("Oh noez");
            }

            finalItem.top = newTop;
          }
          finalItem.bottom = finalItem.top + finalItem.height;
        }
      } while (++shifts < this.options.maxStackHeight && collidingItem);

      if (collidingItem) {
        //We have a collision given max stack height.
        //Find the item where the overlap is smallest.

        var min = overlaps[0];
        for (var j = 1; j < overlaps.length; j++) {
          if (overlaps[j].item.item.data.noStack) continue; //Don't consider collisions with unstackable items
          if (this.getOverlap(finalItem, overlaps[j].item) < this.getOverlap(finalItem, min.item)) {
            min = overlaps[j];
          }
        }
        //finalItem.item.dom.style.zIndex = min.item.item.dom.zIndex;
        finalItem.top = min.top;
        finalItem.bottom = finalItem.top + finalItem.height;
      }
    }
...
links.Timeline.prototype.stackItemsCheckOverlap = function (items, itemIndex,
  itemStart, itemEnd) {
  var eventMargin = this.options.eventMarginX || this.options.eventMargin,
...
links.Timeline.prototype.collision = function (item1, item2, margin) {
  ...
  //if( item1.item.data.noStack || item2.item.data.noStack ) return false;
...
links.Timeline.prototype.getOverlap = function (x, y) {
  if (x.left < y.right) {
    return Math.min(x.right, y.right) - x.left;
  } else if (x.right > y.left) {
    return x.right - Math.max(x.left, y.left);
  }
  return 0;
}; /**
 * fire an event
...
links.Timeline.ClusterGenerator.prototype.getClusters = function (scale) {
  var level = -1,
      ...
      maxItems = 1; // TODO: do not hard code maxItems

  var itemWidth = 40;

  /*if( scale > 8e-7 ) maxItems = 1000;
  ...
  if (!clusters) {
    ...
    //px per millisecond		
    this.timeline.reflowAxis();
    this.timeline.recalcConversion();
    //var ppm = this.timeline.conversion.factor;		
    ...
    for (var groupName in this.groups) {
      if (this.groups.hasOwnProperty(groupName)) {
        ...
        var semiGroups = {};
        for (var i = 0; i < items.length; i++) {
          semiGroups[items[i].data.clusterGroup] = true;
          delete items[i].cluster;
        }

        for (var sg in semiGroups) {
          var i = 0;

          while (i < iMax) {

            //if( items[i].data.noCluster ) { ++i; continue; }
            if (items[i].data.noCluster || items[i].data.clusterGroup != sg || items[i].cluster) {
              ++i;
              continue;
            }

            var clusterItems = [items[i]];
            var start = 1 * items[i].start;
            var min = start, max = start, avg = start;

            for (var j = i + 1; j < iMax; j++) {
              //if( items[j].data.noCluster ) { continue; }
              if (items[j].data.noCluster || items[j].data.clusterGroup != sg || items[j].cluster) {
                continue;
              }

              var other = 1 * items[j].start;
              if (this.timeline.timeToScreen(other) - this.timeline.timeToScreen(avg) <= itemWidth) {
                clusterItems.push(items[j]);
                max = other;
                avg = (avg * (clusterItems.length - 1) + other) / clusterItems.length;
              } else {
                break;
              }
            }
            ...
            if (clusterItems.length > 1) {
              var group = items[i].group ? item.group.content : undefined;
              cluster = this.timeline.createItem({
                'start': new Date(avg),
                'group': group,
                clusterGroup: sg
              });
              cluster.range = { avg: avg, min: min, max: max };
              ...
              //cluster.content = this.timeline.timeToScreen(avg);
              cluster.content = this.timeline.options.clusterFormatter(cluster);

              clusters.push(cluster);
            }

            i += clusterItems.length;
          }
        }
      }
    }
...
links.Timeline.addEventListener = function (element, action, listener, useCapture) {
  if (element.addEventListener) {
    ...
    if (window.navigator.msPointerEnabled) {
      action = action == "touchstart" ? "MSPointerDown" : action == "touchend" ? "MSPointerUp" : action;
    }
