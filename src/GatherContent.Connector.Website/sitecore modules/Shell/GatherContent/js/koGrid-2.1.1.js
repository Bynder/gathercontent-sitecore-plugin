...
(function($) {
    ...
    (function(window) {
        ...
        window.kg.utils = {
            ...
            evalProperty: function(entity, path) {
                ...
                var propPath = path.split('.'),
                    i = 0;
                var tempProp = ko.utils.unwrapObservable(e[propPath[i]]),
                    links = propPath.length;
                i++;
        ...
        ko.bindingHandlers['koGrid'] = (function() {
            return {
                'init': function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                    ...
                    var elem = $(element);
                    options.gridDim = new window.kg.Dimension({
                        outerHeight: ko.observable(elem.height()),
                        outerWidth: ko.observable(elem.width())
                    });
        ...
        window.kg.Column = function(config, grid) {
            ...
            self.groupedByClass = ko.computed(function() {
                return self.isGroupedBy() ? "kgGroupedByIcon" : "kgGroupIcon";
            });
        ...
        window.kg.EventProvider = function(grid) {
            ...
            self.onGroupMouseDown = function(event) {
                ...
                if (groupItem[0].className != 'kgRemoveGroup') {
                    ...
                    if (groupItemScope) {
                        ...
                        self.groupToMove = {
                            header: groupItem,
                            groupName: groupItemScope,
                            index: groupItemScope.groupIndex() - 1
                        };
                    }
                } else {
                    self.groupToMove = undefined;
            ...
            self.onRowMouseDown = function(event) {
                ...
                if (rowScope) {
                    ...
                    window.kg.eventStorage.rowToMove = {
                        targetRow: targetRow,
                        scope: rowScope
                    };
                }
        ...
        window.kg.RowFactory = function(grid) {
            ...
            self.buildEntityRow = function(entity, rowIndex) {
                var row = self.rowCache[rowIndex];
                if (!row) {
        ...
        window.kg.Grid = function(options) {
            var defaults = {
                    ...
                    sortInfo: ko.observable(undefined),
                    multiSelect: true,
                    tabIndex: -1,
            ...
            self.resizeOnData = function(col) {
                ...
                $.each(arr, function(index, elem) {
                    ...
                    if (index === 0) {
                        ...
                        i = window.kg.utils.visualLength(kgHeaderText) + 10;
                    } else {
                        var ngCellText = $(elem).find('.kgCellText');
                        i = window.kg.utils.visualLength(ngCellText) + 10;
                    }
                    if (i > longest) {
                        longest = i;
                    }
                });
                col.width = longest = Math.min(col.maxWidth, longest + 7);
                window.kg.domUtilityService.BuildStyles(self);
            };
            ...
            self.maxRows = ko.observable(Math.max(self.config.pagingOptions.totalServerItems() || self.sortedData().length));
            ...
            self.maxPages = ko.computed(function() {
                self.maxRows(Math.max(self.config.pagingOptions.totalServerItems() || self.sortedData().length));
        ...
        window.kg.StyleProvider = function(grid) {
            ...
            grid.headerStyle = ko.computed(function() {
                return {
                    "width": Math.max(0, grid.rootDim.outerWidth() - 15) + "px",
                    "height": grid.config.headerRowHeight + "px"
                };
            });
        ...
        window.kg.domUtilityService = {
            ...
            BuildStyles: function(grid) {
                ...
                css = "." + gridId + " .kgCanvas { width: " + trw + "px; }" + "." + gridId + " .kgRow { width: " + trw + "px; }" + "." + gridId + " .kgCell { height: " + rowHeight + "px; }" + "." + gridId + " .kgCanvas { width: " + trw + "px; }" + "." + gridId + " .kgHeaderCell { top: 0; bottom: 0; }" + "." + gridId + " .kgHeaderScroller { width: " + (trw + window.kg.domUtilityService.scrollH + 2) + "px}";
                $.each(cols, function(i, col) {

                    css += "." + gridId + " .col" + i + " { width: " + col.width + "px; left: " + sumWidth + "px; right: " + (trw - sumWidth - col.width) + "px; height: " + rowHeight + "px }" + "." + gridId + " .colt" + i + " { width: " + col.width + "px; }";
                    sumWidth += col.width;
                });
                if (window.kg.utils.isIe) {
