...
namespace Bynder.Content.SitecoreConnector.Web.Commands
{
    public class UpdateCommand : Command
    {
        ...
        protected void Run(ClientPipelineArgs args)
        {
            ...
            var uri = "/sitecore modules/shell/contentworkflow/update/update.html";
            var path = string.Format("{0}?id={1}&l={2}&v={3}&db={4}", uri, id, language, version, db);

#if SC72
            Context.ClientPage.ClientResponse.Broadcast(Context.ClientPage.ClientResponse.ShowModalDialog(path, "1600", "700", "Pull Updated Content from Content Workflow", false), "Shell");
#else
            var options = new ModalDialogOptions(path)
            {
                ...
                MinHeight = "400",
                Maximizable = false,
                Header = "Pull Updated Content from Content Workflow"
            };
