...
define(['sitecore', 'dynatree'], function (_sc) {
    ...
    var control = {
        ...
        [
          ...
          { name: "rootItemId", added: true },
          { name: "database", added: true },
          { name: "whitelist", added: true },
        ...
        view:
        {
            initialized: function () {
                ...
                var clickFolderModeAttribute = this.$el.attr("data-sc-option-clickfoldermode"),
                  ...
                  whiteListAttribute = this.$el.attr('data-sc-whitelist');

                // additonal model attributes
                this.model.set("rootItemId", this.$el.attr('data-sc-rootitem'));
                this.model.set("database", this.$el.attr('data-sc-database'));

                if (whiteListAttribute === '*') {
                    this.model.set("whitelist", whiteListAttribute);
                } else {
                    this.model.set("whitelist", whiteListAttribute ? whiteListAttribute.split('|') : []);
                }
                ...
                this.model.set("pathToLoad", this.$el.attr("data-sc-loadpath"));
                
                this.buildTree();

                // extended events
                this.model.on('change:rootItemId', function(ctx) {
                    ctx.viewModel.buildTree();
                }, this);

                // bind deactivation handler to tree when cancelled
                this.model.on('change:deactivateHandler', function() {
                    var handler = this.model.get('deactivateHandler');
                    var wrappedHandler;
                    if (handler && _.isFunction(handler)) {
                        wrappedHandler = jQuery.proxy(function (flag, node) {
                            var returnVal = handler(flag, node);
                            if (returnVal === false) {
                                this.model.trigger("cancel:activate");
                            }
                            return returnVal;
                        }, this);
                    } else {
                        wrappedHandler = function () { return true; }
                    }

                    this.widget.apply(this.$el, ["option", "onQueryActivate", wrappedHandler]);
                }, this);
            },

            buildTree: function() {
                var root = this.model.get('rootItemId');
                ...
                var self = this;
                var databaseUri = new _sc.Definitions.Data.DatabaseUri(this.model.get('database'));
                var database = new _sc.Definitions.Data.Database(databaseUri);
                database.getItem(this.model.get('rootItemId'), function(item) {
                    var rootNode = {
                    ...
                    if (self.model.get("showSitecoreIcons")) {
                        ...
                        rootNode.icon = item.$icon;
                    }
                    if (self.widget) {
                        self.widget.apply(self.$el, ["getRoot"]).addChild(rootNode);
                    }

                    if (self.model.get("pathToLoad") && self.model.get("pathToLoad") !== "") {
                        self.loadKeyPath();
                    }
                });
            ...
            nodeExpanding: function (node) {
                ...
                database.getChildren(itemUri.getItemId(), function (items) {
                    ...
                    var whitelist = self.model.get('whitelist');
                    if (whitelist != '*') {
                        filteredItems = _.reject(filteredItems, function(child) {
                            return !_.contains(whitelist, child.itemId);
                        });
                    }
